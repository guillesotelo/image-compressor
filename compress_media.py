...
def savings_pct(bytes_in: int, bytes_out: int) -> float:
    """Percent of the original size saved. Positive = smaller than the
    original, negative = the "compressed" file actually grew."""
    if not bytes_in:
        return 0.0
    return (bytes_in - bytes_out) * 100.0 / bytes_in


def savings_text(pct: float) -> str:
    """Human wording for a savings percentage: '20% smaller' / '21% larger'."""
    return f"{pct:.0f}% smaller" if pct >= 0 else f"{-pct:.0f}% larger"
...
def probe_video(path: Path) -> dict:
    """First video stream of `path`, plus "_duration".
    {} if ffprobe could not read the file at all; {"_no_video": True, ...} if it
    read it fine but there is no video stream (audio-only or empty container)."""
    ...
    try:
        ...
        stream = dict(streams[0]) if streams else {"_no_video": True}
...
class RunLog:
    ...
    def file(self, status: str, kind: str, rel: str, src: Path, out: Path | None,
             ...
             note: str = ""):
        pct = savings_pct(bytes_in, bytes_out) if (bytes_in and bytes_out) else None
        saved = f"{pct:.1f}" if pct is not None else ""
        self._csv.writerow([f"{datetime.now():%Y-%m-%d %H:%M:%S}", status, kind, rel,
                            str(src), str(out) if out else "", bytes_in or "",
                            bytes_out or "", saved, f"{seconds:.1f}", error or note])
        self._csv_fh.flush()

        detail = f"{human(bytes_in)} -> {human(bytes_out)} ({savings_text(pct)})" \
            if pct is not None else human(bytes_in) if bytes_in else ""
...
def main():
    ...
    p.add_argument("--allow-larger", action="store_true",
                   help="Keep the re-encoded file even when it ends up bigger "
                        "than the original (default: keep the original instead)")
    ...
    total_in = total_out = done = failed = skipped = copied = kept = 0
    run_start = time.time()
    interrupted = False
        ...
        with Progress(
            ...
            for f in files:
                ...
                if args.skip_existing:
                    existing = next((c for c in dict.fromkeys([out, dst_root / rel])
                                     if c.exists() and c.stat().st_size > 0), None)
                    if existing:
                        skipped += 1
                        if log:
                            log.file("skipped", kind, str(rel), f, existing,
                                     f.stat().st_size, existing.stat().st_size, 0.0)
                ...
                if is_video:
                    ...
                    if info.get("_no_video"):
                        # Video extension, but no video stream inside (audio-only
                        # .mov/.m4v, empty or truncated container): copy it as-is
                        # instead of feeding ffmpeg something it cannot encode.
                        is_video, kind = False, "other"
                        probe_note = "no video stream found - copied as-is"
                ...
                if ok:
                    ...
                    recompressed = not (kind == "other" or heic_passthrough)
                    kept_original = (recompressed and not args.allow_larger
                                     and size_out >= size_in)
                    if kept_original:
                        # Re-encoding grew the file (already-compressed source):
                        # throw the new one away and keep the original.
                        grew, tmp_size = savings_pct(size_in, size_out), size_out
                        tmp.unlink(missing_ok=True)
                        if out != dst_root / rel:
                            out.unlink(missing_ok=True)   # stale .jpg from an older run
                            out = dst_root / rel
                        tmp = temp_target(out)
                        shutil.copy2(f, tmp)
                    ...
                    total_in += size_in
                    total_out += size_out
                    pct = savings_pct(size_in, size_out)
                    note = probe_note
                    if kept_original:
                        kept += 1
                        status = "kept-original"
                        note = (f"re-encode was {savings_text(grew)} "
                                f"({human(tmp_size)}), original kept")
                        progress.console.print(
                            f"  [dim]{rel}[/]  {human(size_in)} → [yellow]kept original[/] "
                            f"[dim](re-encode came out {savings_text(grew)})[/]")
                    elif recompressed:
                        done += 1
                        status = "ok"
                        col = "green" if pct >= 0 else "yellow"
                        progress.console.print(
                            f"  [dim]{rel}[/]  {human(size_in)} → [{col}]{human(size_out)}[/] "
                            f"[bold {col}]({savings_text(pct)})[/]")
                    else:
                        copied += 1
                        status = "copied"
                    if log:
                        log.file(status, kind, str(rel), f, out, size_in, size_out,
                                 elapsed, note=note)
                else:
                    failed += 1
                    if log:
    ...
    saved_pct = savings_pct(total_in, total_out)
    style = "yellow" if (failed or interrupted) else "green"
    col = "green" if saved_pct >= 0 else "yellow"

    console.print(Panel.fit(
        f"[bold]{done}[/] compressed, [dim]{copied} copied[/], "
        f"[dim]{kept} kept original[/], [red]{failed}[/] failed, "
        f"[dim]{skipped} skipped[/]\n"
        f"[bold]{human(total_in)}[/] → [bold {col}]{human(total_out)}[/]   "
        f"[bold {col}]{savings_text(saved_pct)} overall[/]   "
        f"[dim]in {elapsed_total/60:.1f} min[/]\n"
        f"[dim]{dst_root}[/]",
    ...
    if log:
        log.block(
            ...
            f"saved      : {saved_pct:.1f}% ({savings_text(saved_pct)})\n"
